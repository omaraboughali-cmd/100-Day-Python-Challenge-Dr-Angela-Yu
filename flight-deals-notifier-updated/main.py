...
for destination in sheet_data:
    ...
    print(f"\n✈️  Checking flights to {city_name} ({dest_code})...")
    ...
    if cheapest_flight.price < target_price:
        ...
        msg = (
            ...
            f" \nDEAL FOUND for {city_name}!\n"

            f"\n   Price: ${cheapest_flight.price} (Target: ${target_price})\n"

            f"\n   Fly from {cheapest_flight.origin_airport} to {cheapest_flight.destination_airport}\n"

            f"\n   Departure time : {cheapest_flight.outbound_date} , Arrival time : {cheapest_flight.return_date}\n"
        ...
        for user in data_manager.user_data:
            ...
            print(f'\nAlert for {user['firstName']}')
            notification_manager.send_email(user['email'] , email_message)
